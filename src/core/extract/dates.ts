 ...
 *
 * ## THIS LIST IS ENGLISH, AND THE TWO READERS PAY DIFFERENTLY FOR IT
 *
 * An adopting institution cataloguing in another language should know what it
 * costs before it finds out. HERE it is harmless: a date this cannot see leaves
 * a cell empty and the row is flagged, which is the same outcome as a document
 * that states no date. In `verify.ts` it is not: an unreadable date is a
 * document that supports nothing, so a CORRECT day-precision description
 * generated by a language model is refused and discarded -- `Falleció el 6 de
 * enero de 2024` and `Er starb am 6. Januar 2024` both state the day, and both
 * read there as a bare `2024`.
 *
 * Refusing is the safe direction and stays; what `verify.ts` must never do is
 * report it as "the document states no such date", which is a claim about the
 * document rather than about what this list can read. Adding a language is
 * adding its month names here -- both recognisers are built from this array --
 * and its numeral words to `verify.ts`'s `SMALL`. Note the three-letter stem
 * must stay distinct across the list: both patterns abbreviate on it.
 */
