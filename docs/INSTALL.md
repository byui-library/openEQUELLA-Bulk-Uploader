...
- **`openEQUELLA Bulk Uploader 1.0.0.exe`** — a single file. Double-click it
  to run; nothing is installed anywhere.
- **`openEQUELLA Bulk Uploader Setup 1.0.0.exe`** — an installer. Adds a Start
...
than 1.0.0, take that one — everything below still applies.
